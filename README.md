...
- [x] MDL animation playback — `MdlModel::pose(sequence, frame)` decodes each sequence's compressed per-bone animation tracks (RLE keyframe streams, GoldSrc's `mstudioanim_t`/`mstudioanimvalue_t` format) and re-skins the mesh at any fractional frame, interpolated between the two nearest keyframes. The view model now actually plays idle/draw/shoot/reload instead of standing in its static bind pose, switching sequences on the real fire/reload/equip events and falling back to idle when a one-shot animation finishes. Verified by rendering both a weapon (v_ak47.mdl idle/shoot, hand+gun bones) and a full player model (urban.mdl walk cycle, opposite mid-stride frames) at multiple frames — all coherent, non-garbled poses — plus a live in-engine screenshot showing the view model in a genuinely different pose after a simulated shot. Limitations: only a sequence's first blend is used (multi-blend 9-way aim-direction sequences still play, just without directional blending), and only sequences embedded in the main file are supported (true of every sequence in this game's asset set, but not of exotic external-seqgroup models like the hostage NPCs, which already fail to load for unrelated reasons). Player-model (non-view-model) animation isn't wired into any renderer yet — there's no third-person rendering to drive.
