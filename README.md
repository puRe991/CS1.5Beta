...
- [x] Bot navigation: a coarse waypoint graph (`nav.h`/`nav.cpp`'s `NavGraph`) built once per map from spawn points, bomb-target/buy-zone centers, and a floor-probed grid sample, connected into a visibility graph and searched with A* — bots path-find around obstacles instead of steering straight at a target when they can't already see it (`Idle`/`Search` movement; `Chase`/`Attack` still steer directly, since those states only exist because the bot already has a clear sightline). Not a real polygonal navmesh (no hull-width-aware clearance, no ledges/jump links, no per-node cost besides distance) and nodes/edges are capped for build-time safety, so very large or geometrically complex maps get sparser coverage — see `nav.cpp`'s `kMaxNodes`.
- [x] Bot tactical behavior (partial): buy decisions (`bots.cpp`'s `buyWeapon()` — spends the bot's money on the priciest affordable primary, falling back to the priciest affordable pistol; no eco/force-buy/save reasoning, no armor/utility purchases) and simple objective-awareness (idle bots patrol toward a bomb site instead of wandering purely at random, once the nav graph can route them there). Still open: no attacker/defender role split (both teams patrol sites the same way), no rotations, no plant/defuse by bots (only the player can touch the bomb), no utility usage (bots don't have or throw grenades — none exist yet, see Grenades & thrown utility), and no explicit coordination beyond the one shared "last saw the player here" callout hearing already provides.
